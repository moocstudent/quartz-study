...
public class SimpleTriggerExample {
	...
	private static final String job_name = "liuzhihu_job2";
	private static final String job_group = "liuzhihu_job_group2";

	private static final String trigger_name = "liuzhihu_trigger2";
	private static final String trigger_group = "liuzhihu_trigger_group2";
	...
	public static void main(String[] args) throws Exception {
		...
		example.start();
		// 恢复
		// example.resumeJob("liuzhihu_job1", "liuzhihu_group1");
	}
	 ...
	 *            trigger组名
	 * @param afterTime
	 *            当前时间 之后多少秒执行( 1*1000L : 1s)
	 * @throws Exception
	 */
	public void register(String jobName, String jobGroup, String triggerName, String triggerGroup, long afterTime)
			throws Exception {
		...
		Trigger trigger = newTrigger().withIdentity(triggerName, triggerGroup)
				.withSchedule(SimpleScheduleBuilder.simpleSchedule()).startAt(runTime).build();
	...
	public void start() {
		try {
			...
			/*
			 * // 等待65秒 log.info("------- Waiting 65 seconds... -------------");
			 * try { // wait 65 seconds to show job Thread.sleep(5L * 1000L); //
			 * executing... } catch (Exception e) { // }
			 * 
			 * // 关闭调度器 log.info("------- Shutting Down ---------------------");
			 * sched.shutdown(true); log.info(
			 * "------- Shutdown Complete -----------------");
			 */
