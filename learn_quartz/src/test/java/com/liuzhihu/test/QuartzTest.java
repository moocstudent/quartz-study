...
public class QuartzTest {
	...
	public void testAdd() {
		try {
			...
			for (int i = 1; i <= 1; i++) {
				...
				JobDetail job = newJob(MyJob.class).withIdentity("liuzhihu_job" + i, "liuzhihu_group").build();
				job.getJobDataMap().put("jobName", "liuzhihu_job" + i);
				// SimpleScheduleBuilder schedBuilder =
				// SimpleScheduleBuilder.repeatSecondlyForTotalCount(10);
				// 定义触发器，在下一分钟启动
				// .withSchedule(schedBuilder).startAt(runTime).build();
				String cron = "0/" + ((i * 2) + 1) + " * * * * ?";
				Trigger trigger = newTrigger().withIdentity("liuzhihu_trigger" + i, "liuzhihu_trigger_group")
	...
	public void testRunOnceTrigger() {
		...
		try {
			...
			TriggerKey tk = new TriggerKey("liuzhihu_trigger1", "liuzhihu_trigger_group");
